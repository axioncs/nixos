...
local function extract_local_frame(filepath)
	...
	-- Use a fixed, stable path so the companion binary can reliably read the
	-- frame. os.tmpname() produced a fresh name per extraction which raced with
	-- the binary reading user-data/discord-thumbnail (file gone / replaced).
	local tmpfile = "/tmp/mpv-discord-thumb.jpg"
